    ...
    const generatePriceData = () => {
      ...
      for (let i = count - 1; i >= 0; i--) {
        ...
        if (timeRange === "today") {
          ...
          timeLabel = date.toLocaleDateString("fa-IR", { month: "numeric", day: "numeric" })
        }
      ...
      <div className="flex-1 lg:mr-64">
        <main className="container mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gold">تغییرات قیمت طلا</h1>
              <p className="text-cream/60">نمایش تغییرات قیمت خرید و فروش طلا</p>
            </div>
          </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-400 mt-1">
                      {priceData[priceData.length - 1]?.buyPrice.toLocaleString("fa-IR")} ریال
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-400 mt-1">
                      {priceData[priceData.length - 1]?.sellPrice.toLocaleString("fa-IR")} ریال
                    </p>
                  </div>
            ...
            <CardHeader>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="text-cream/60">نمایش تغییرات قیمت در بازه زمانی انتخابی</CardDescription>
                </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gold/20">
                      <th className="text-right py-3 px-4 text-gold font-semibold">زمان</th>
                      <th className="text-right py-3 px-4 text-gold font-semibold">قیمت خرید</th>
                      <th className="text-right py-3 px-4 text-gold font-semibold">قیمت فروش</th>
                      <th className="text-right py-3 px-4 text-gold font-semibold">تغییرات (%)</th>
                      <th className="text-right py-3 px-4 text-gold font-semibold">نمودار</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1 h-8">
                            {Array.from({ length: 10 }).map((_, i) => {
                              const height = Math.abs(record.change) * 10 + Math.random() * 20
                              return (
                                <div
                                  key={i}
                                  className={`w-1 rounded-full ${
                                    record.change >= 0 ? "bg-green-400/50" : "bg-red-400/50"
                                  }`}
